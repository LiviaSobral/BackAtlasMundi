...
router.post('/me', authMiddleware, modifyMiddleware, ModPermissionMiddleware, validateDTO(CreateCountryDTO), controller.create.bind(controller)) // post de pais para criar um pais, so entra nessa rota se passar pelo modifyMiddleware que verifica se o usuario fazendo a requisição é um professor. O body precisa de todas as informações necessarias para criar um pais (nome{apenas com letras}, informações rapidas, url{opcional para imagens}, politics e politics2{separados para facilitar o front},history e history2, culture e culture2, inEnglish{boolean pra saber se esta em ingles}, sources{a fonte das informações} )
router.put('/', controller.getByName.bind(controller)) // get do pais, precisa de nome e o boolean de ingles {obrigatorio ter os dois ja que um nome pode ser igual em ingles e portugues}
router.put('/me', authMiddleware, modifyMiddleware, ModPermissionMiddleware, validateDTO(UpdateCountryDTO), controller.update.bind(controller)) // Update do pais, tambem passa pelo modifyMiddleware que verifica se o usuario fazendo a requisição é um professor, se sim pode receber qualquer das informações do create
router.delete('/me', authMiddleware, modifyMiddleware, ModPermissionMiddleware,controller.remove.bind(controller)) // delete passa pelo modifyMiddleware pra verificar permição e deleta conforme o Nome e Linguagem
